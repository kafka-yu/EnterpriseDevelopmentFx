...
namespace NkjSoft.BLL
{
    ...
    public class HomeBLL : IHomeBLL, IDisposable
    {
        ...
        public string GetNode(SysMenu item, bool isLeaf = false)
        {
            ...
            if (isLeaf)
            {
                return string.Format("<li data-options=@{0}@><a href=@javascript:void(0);@ icon=@{1}@ rel=@{2}@ id=@{3}@>{4}</a></li>^", string.Join(",", dataoptions), item.Iconic, item.Url, item.Id, item.Name);
            }
            else
            {
