...
namespace NkjSoft.ORM.Data.Common
{
    ...
    public class QueryBinder : DbExpressionVisitor
    {
                ...
                switch (m.Method.Name)
                {
                    case "Insert":
                        return this.BindInsert(
                            ...
                            m.Arguments.Count > 3 ? GetColumnIndeedExpression(m.Arguments[3]) : null
                            ); //TODO: 2010-6-20 ....解决指定列的问题
                    case "Update":
                        return this.BindUpdate(
                            ...
                            m.Arguments.Count > 4 ? GetColumnIndeedExpression(m.Arguments[4]) : null
                            );
                    case "InsertOrUpdate":
        ...
        //解决: 指定需要更新的列。
        /// <summary>
        /// Gets the column indeed expression.
        /// </summary>
        /// <param name="e">The e.</param>
        /// <returns></returns>
        private ColumnIndeedExpression GetColumnIndeedExpression(Expression e)
        {
            while (e.NodeType == ExpressionType.Quote)
            {
                e = ((UnaryExpression)e).Operand;
            ...
            if (e.NodeType == ExpressionType.Constant)
            {
                //ColumnIndeedExpression temp = new ColumnIndeedExpression(new ColumnsIndeed(){ ColumnsToBeHandled=((ConstantExpression)e).Value);//as ColumnIndeedExpression;
                ColumnsIndeed temp = ((ConstantExpression)e).Value as ColumnsIndeed;

                return new ColumnIndeedExpression(temp);
                //return ((ConstantExpression)e).Value as ColumnIndeedExpression;
            }
            return e as ColumnIndeedExpression;
        }
