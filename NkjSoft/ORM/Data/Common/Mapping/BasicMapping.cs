...
namespace NkjSoft.ORM.Data.Common
{
    ...
    public abstract class BasicMapping : QueryMapping
    {
        ...
        public virtual bool IsRelationshipSource(MappingEntity entity, MemberInfo member)
        {
        ...
        public override IEnumerable<MemberInfo> GetMappedMembers(MappingEntity entity)
        {
            ...
            HashSet<MemberInfo> members = new HashSet<MemberInfo>(
                type.GetProperties().Cast<MemberInfo>().Where(m => this.IsMapped(entity, m) && m.GetAttributeValue<ColumnAttribute, bool>(p => p.Ignore == false)));
            //members.UnionWith(type.GetProperties().Cast<MemberInfo>().Where(m => this.IsMapped(entity, m)));

            if (members.Count == 0)
            {
                members = new HashSet<MemberInfo>(type.GetProperties().Cast<MemberInfo>());
            }
    ...
    public class BasicMapper : QueryMapper
    {
        ...
        protected virtual Expression BuildEntityExpression(MappingEntity entity, IList<EntityAssignment> assignments)
        {
            ...
            if (readonlyMembers.Length > 0 || !hasNoArgConstructor)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format("Cannot construct type '{0}' with all mapped included Members.", entity.ElementType));
                }
        ...
        /// <summary>
        /// Determines whether [is null relationship assignment] [the specified entity].
        /// </summary>
        /// <param name="entity">The entity.</param>
        /// <param name="assignment">The assignment.</param>
        /// <returns>
        /// 	<c>true</c> if [is null relationship assignment] [the specified entity]; otherwise, <c>false</c>.
        /// </returns>
        ...
        {
            return GetInsertExpression(entity, instance, selector, null);
        }
        ...
        protected virtual Expression GetInsertResult(MappingEntity entity, Expression instance, LambdaExpression selector, Dictionary<MemberInfo, Expression> map)
        {
            ...
            if (generatedIds.Count > 0)
            {
                ...
                if (mex != null && this.mapping.IsPrimaryKey(entity, mex.Member) && this.mapping.IsIdentity(entity, mex.Member))
                {
        ...
        {
            var mm = this.mapping.GetMappedMembers(entity).ToList();//m => this.mapping.IsPrimaryKey(entity, m)m.GetAttributeValue<ColumnAttribute, bool>(c => c.IsPrimaryKey));//
            var ik = mm.Where(m => m.GetAttributeValue<ColumnAttribute, bool>(c => c.IsPrimaryKey)).ToList();
            var ge = ik.Select(m => this.GetMemberExpression(root, entity, m).Equal(Expression.MakeMemberAccess(instance, m)));
            var r = ge.Aggregate((x, y) => x.And(y));

            return r;
        }

        /// <summary>
        /// Gets the entity exists test.
        /// </summary>
        /// <param name="entity">The entity.</param>
        /// <param name="instance">The instance.</param>
        /// <returns></returns>
        ...
        {
            return GetUpdateExpression(entity, instance, updateCheck, selector, null, @else);
        }
